...
#-------------------------------------------Diagnostic Plot (Plot 4,5) #### Cook's Distance, Leverage
par(mfrow=c(1,1))
plot(fit,which=4)
...
y_pred <- (1+(predict(fit_modified_after_boxcox)*best_lam))^(1/best_lam)
plot(housing_remove$medv,y_pred,xlab = "y_true (Original)",ylab="y_pred(Prediction)",main = "Modified Data (After Removing Outlier) - Box Cox (Reverse)")
