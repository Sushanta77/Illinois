...
confint(cat_model,interval="confidence",level=0.90)["Bwt",]
confint(cat_model,interval="confidence",level=0.99)["(Intercept)",]
...
- A statistical decision at $\alpha = 0.05$ 
    ...
    The P value for $\beta_1$ which is: **`r summary(cat_model)$coefficient[,"Pr(>|t|)"][2]`** < **0.05** ($\alpha$). Since P is smaller than $\alpha$, we **reject the null Hypothesis**

- A conclusion in the context of the problem:

    Since we **reject the null hypothesis**, hence $\beta_1$ is non zero, so there is **significant linear relationship** Cat's Body Weight and Cat's Heart Weight


**(b)** Calculate a 90% confidence interval for $\beta_1$. Give an interpretation of the interval in the context of the problem.

90% confident interval for $\beta_1$ is between **`r confint(cat_model,level=0.90)["Bwt",][1]`** and **`r confint(cat_model,level=0.90)["Bwt",][2]`**

**Interpretation:** We are 90% confident that true change in mean of Cat's Heart Weight for an increase Cat's Body Weight by 1 kg is between **`r confint(cat_model,level=0.90)["Bwt",][1]`** and **`r confint(cat_model,level=0.90)["Bwt",][2]`**


**(c)** Calculate a 99% confidence interval for $\beta_0$. Give an interpretation of the interval in the context of the problem.

99% confidence interval for $\beta_0$ is between **`r confint(cat_model,level=0.99)["(Intercept)",][1]`** and **`r confint(cat_model,level=0.99)["(Intercept)",][2]`**

**Interpretation:** We are 99% confident that the true change in mean of Cat's Heart Weight for zero Cat's Body is between **`r confint(cat_model,level=0.90)["(Intercept)",][1]`** and **`r confint(cat_model,level=0.90)["(Intercept)",][2]`**
...
99% confidence interval of mean Body weight of **2.1** is between **`r predict_newdata1[1,]["lwr"]`** and **`r predict_newdata1[1,]["upr"]`**

99% confidence interval of mean Body weight of **2.8** is between **`r predict_newdata1[2,]["lwr"]`**  and **`r predict_newdata1[2,]["upr"]`**


**2.1** have **wider** interval than that of **2.8**, becase the range of **2.1** is **`r predict_newdata1[1,]["upr"] - predict_newdata1[1,]["lwr"]`** > **`r predict_newdata1[2,]["upr"] - predict_newdata1[2,]["lwr"]`** of **2.8**


**(e)** Use a 99% prediction interval to predict the heart weight for body weights of 2.8 and 4.2 kilograms.

99% confidence interval of mean Body weight of **2.8** is between **lower:`r predict_newdata2[1,]["lwr"]`** and **`r predict_newdata2[1,]["upr"]`**

99% confidence interval of mean Body weight of **4.2** is between **lower:`r predict_newdata2[2,]["lwr"]`** and **`r predict_newdata2[2,]["upr"]`**
...
plot(Hwt~Bwt,
     ...
     main = "Scatter Plot Cat's Body Weight Vs Cat's Heart Weight",
     col = "grey",
     pch = 20,
     cex = 2
     )
abline(cat_model,lwd = 5, col = "darkorange")
lines(bwt_grid,hwt_ci_band[,"lwr"],col="dodgerblue",lwd=2,lty=2)
lines(bwt_grid,hwt_ci_band[,"upr"],col="dodgerblue",lwd=2,lty=2)
lines(bwt_grid,hwt_pi_band[,"lwr"],col="dodgerblue",lwd=2,lty=3)
lines(bwt_grid,hwt_pi_band[,"upr"],col="dodgerblue",lwd=2,lty=3)
```
...
- A statistical decision at $\alpha = 0.05$ 
    ...
    Since the p-value of the test for $\beta_1$ is : **`r beta_1_hat_p`** > 0.05 (=$\alpha$), hence we **failed to reject the Hypothesis** of $\beta_1$ = 4, where the slope ($\beta_1$) is close to the value of 4
...
- A conclusion in the context of the problem
    ...
    Since we **failed to reject the null hypothesis**, $\beta_1$ close to be 0, hence there is **no significant linear relationship** exists between ozone and wind speed of Ozone's dataset
...
- A conclusion in the context of the problem
    ...
    Since we **reject the null hypothesis**, $\beta_1$ $\neq$ 0, hence there is **significance linear relationship**  exists between ozone and temperature of the Ozone's dataset
