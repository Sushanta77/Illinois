...
###Introduction
In this simulation we will investigate the significance of the regression test. We will simulate 2 different model

Below is the **1st model**, which is the **Signifianct model**
\[ Y_i = \beta_0 + \beta_1 x_{i1} + \beta_2 x_{i2} + \beta_3 x_{i3} + \epsilon_i.\]

where $\epsilon_i$ follows a Normal distribution of mean = 0 and Varience = $\sigma$ = 
...
We need to simulate 2500 simulations for both the models for each value of sigma, so around 2500 X 3 = 7500 times for each model, so TOTAL 2 X 7500 = 15,000 times for both the model

We need to show there are around 15000 times the model is being trained as part of the simulations
...
#####Do we know the true distribution
######True distribution of the F Test
```{r}
...
######True distribution of the P value
```{r}
par(mfrow=c(2, 2))
...
######True distribution of the R squared
```{r}
...
As per the above model, the correct model is the **Model -6 **, we need to see whether this procedure is correct enough to get the correct model
...
As part of this exercise, the following are the Goals to achieve
 ...
 - Discuss on the following points in the **Disucssion** section
   ...
   - If we do the average of the Test RMSE, whether the simulation methor selects the correct model (i.e. **Model 6**)
...
Below is the code which will runs the 1000 simulations against each sigma (σ)
 - Will Create 9 matrix for Train Set which will stored the Train RMSE, where each columns holds the Train RMSE for each value of σ = 1,2 and 4
 - Will Create 9 matrix for Test Set which will stored the Test RMSE, where each columns holds the Train RMSE for each value of σ = 1,2 and 4
 
 - Will Create 9 matrix which stored the average Train Set RMSE from the Train Set RMESE
 - Will Create 9 matrix which stored the average Test Set RMSE from the Test Set RMSE
...
####Below are the 3 curves we have plotted which shows how Train RMSE and Test RMSE is performed against each model (1/2/3.../7/8/9)
```{r}
...
The above plot shows the Train RMSE and Test RMSE for sigma = 1, is being very close to each other. As the model grow, both the Train / Test RMSE falls, however the tipping point is the ** Model 6**
...
The above plot shows the Train RMSE and Test RMSE for sigma = 2, is being very close to each other till model 6, after model 6 they are ditant apart to each other. Also As the model grow, both the Train / Test RMSE falls, however the tipping point is the ** Model 6**, the Train/Test RMSE increases after it
...
The above plot shows the Train RMSE and Test RMSE for sigma = 4, is being very close to each other till model 2, after model 2 they are ditant apart to each other and distance between them increase as the model grows. Also As the model grow, both the Train / Test RMSE falls, however the tipping point is the ** Model 6**, the Train/Test RMSE increases after it
...
####Does the simulation method always select the correct model
```{r}
...
#Matrix to store the Power of the simulated models
model_sim_5000 = cbind(beta_1_sim_val=rep(0,length(beta_1)),n_10_sigma_1=rep(0,length(beta_1)),n_20_sigma_1=rep(0,length(beta_1)),n_30_sigma_1=rep(0,length(beta_1)),n_10_sigma_2=rep(0,length(beta_1)),n_20_sigma_2=rep(0,length(beta_1)),n_30_sigma_2=rep(0,length(beta_1)),n_10_sigma_4=rep(0,length(beta_1)),n_20_sigma_4=rep(0,length(beta_1)),n_30_sigma_1=rep(0,length(beta_1)))

model_sim_5000[,1] = seq(from=-2,to=2,by=0.1)
extra_counter = 0

#Loop to iterate over Sigma, number of samples, beta_1 values and 5000 simulations to extract and store the Power values
run_counter = 0
for(s in 1:length(sigma)){ #sigma = c(1,2,4)
  for (n_s in 1:length(n)){ #n = c(10,20,30)
    for (b in 1:length(beta_1)){ # beta_1 = seq(from=-2,to=2,by=0.1)
      ...
      power_sim_beta = tot_sum_signifincace_sim / no_loop
      model_sim_5000[b,s+n_s+extra_counter] = power_sim_beta    
      print (paste(run_counter," Sigma:",sigma[s]," n:",n[n_s]," beta_1:",beta_1[b]," Signifiance_Sum:",tot_sum_signifincace_sim, " Power:",power_sim_beta))
    }
  }
  extra_counter = extra_counter + 2
...
plot(model_sim_3000[,1],model_sim_3000[,2], type="o", col="dodgerblue", pch=1, lty=1,main="Sigma = 1,2,4 & n = 10,20,30",xlab = "beta_1",ylab = "Power")
...
#Below code is to plot the simulated beta_1 for 5000 simulations. This plot will helps us to understands, whether there is anything changes because of increase the simulation
#from 3000 iteration to 5000 iteration
plot(model_sim_5000[,1],model_sim_5000[,2], type="o", col="dodgerblue", pch=1, lty=1,main="Sigma = 1,2,4 & n = 10,20,30",xlab = "beta_1",ylab = "Power")
points(model_sim_5000[,1], model_sim_5000[,3], col="red", pch=2,lty=2)
lines(model_sim_5000[,1], model_sim_5000[,3], col="red",lty=2)
points(model_sim_5000[,1], model_sim_5000[,4], col="tan1", pch=3,lty=3)
lines(model_sim_5000[,1], model_sim_5000[,4], col="tan1",lty=3)

points(model_sim_5000[,1],model_sim_5000[,5], type="o", col="yellow", pch=4, lty=4)
lines(model_sim_5000[,1],model_sim_5000[,5], type="o", col="yellow", pch=4, lty=4)

points(model_sim_5000[,1], model_sim_5000[,6], col="deeppink", pch=5,lty=5)
lines(model_sim_5000[,1], model_sim_5000[,6], col="deeppink",lty=5)
points(model_sim_5000[,1], model_sim_5000[,7], col="darkolivegreen4", pch=6,lty=6)
lines(model_sim_5000[,1], model_sim_5000[,7], col="darkolivegreen4",lty=6)

points(model_sim_5000[,1],model_sim_5000[,8], type="o", col="darkgreen", pch=7, lty=7)
lines(model_sim_5000[,1],model_sim_5000[,8], type="o", col="darkgreen", pch=7, lty=7)

points(model_sim_5000[,1], model_sim_5000[,9], col="darkblue", pch=8,lty=8)
lines(model_sim_5000[,1], model_sim_5000[,9], col="darkblue",lty=8)
points(model_sim_5000[,1], model_sim_5000[,10], col="blueviolet", pch=9,lty=9)
lines(model_sim_5000[,1], model_sim_5000[,10], col="blueviolet",lty=9)
