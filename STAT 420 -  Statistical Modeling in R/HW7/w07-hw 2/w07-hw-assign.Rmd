...
```{r}
plot(CO2~horse,data=epa2015,col=plot_colors[type],pch=as.numeric(type))
abline(slr_model,lwd=2,col="green",lty=2)
legend("topright",legend=c("Both","Car","Truck"),col = plot_colors,pch=c(1,2,3))
```
...
```{r}
summary(slr_model)$coefficient[2,1]
```

The estimate for the average change in `CO2` for a one foot-pound per second increase in `horse` for a vehicle of type `car` is : **`summary(slr_model)$coefficient[2,1]`
...
```{r}
predict(slr_model,interval="prediction",level=0.90,newdata=data.frame(horse=148))
```

The 90% prediction interval for the `CO2` of a Subaru Impreza Wagon, which is a vehicle with 148 horsepower and is considered type `Both` by using the Simple Linear Regression is : **`r predict(slr_model,interval="prediction",level=0.90,newdata=data.frame(horse=148))`**
...
The 90% prediction interval for the `CO2` of a Subaru Impreza Wagon, which is a vehicle with 148 horsepower and is considered type `Both` by using the Additive Model is : **`r predict(CO2_add,interval="prediction",level=0.90,newdata=data.frame(horse=148,type='Both'))`**
...
The 90% prediction interval using this model for the `CO2` of a Subaru Impreza Wagon, which is a vehicle with 148 horsepower and is considered type `Both` is **`r predict(CO2_int,newdata = data.frame(horse=148,type="Both"),interval = "prediction", level = 0.90)`**
...
####Anova F Test between Additive vs Interaction Model
```
anova(CO2_add,CO2_int)
anova(CO2_add,CO2_int)$'Pr(>F)'[2]
```
The Anova F test gives the P value as **`r anova(CO2_add,CO2_int)$'Pr(>F)'[2]`** < 0.10 ($\alpha$), where the P value (**`r anova(CO2_add,CO2_int)$'Pr(>F)'[2]`**) is very smaller than of 0.10 ($\alpha$), hence **we reject the Additive Model**, and select the **Interaction Model**. Based on the test and significance level we will preferred the **Interaction Model**.
...
The P values from anova between the Additive model and Interaction model is **`r anova(blood_add,blood_int)$'Pr(>F)'[2]`** > 0.10 ($\alpha$), hence we **failed to reject the additive hypothesis**, hence we prefer the **Additive Model**, i.e. the model without any interaction
...
$$
Y_i = \beta_0 + \beta_1 x_{age} + \beta_2 x_{education} + \beta_3 x_{income}
$$
```{r}
beta_1 = summary(blood_int)$coefficient[2,1]
beta_5 = summary(blood_int)$coefficient[2,1]
(beta_1 + beta_5)
```
Estimate of the change in average `Blood` for a one-unit increase in `Age` for a highly educated, low income, male patient is: **`r  (beta_1 + beta_5)`**
...
Y_i = \beta_0 + \beta_1 x_{Pressure} + \beta_2 x_{Blood} + \beta_3 x_{Rate} + \epsilon \\
Y_i = \beta_0 + \beta_1 x_{Pressure} + \beta_2 x_{Blood} + \beta_3 x_{Rate} + \beta_4 x_{Pressure} x_{Blood} + \beta_5 x_{Pressure} x_{Rate} + \beta_6 x_{Blood} x_{Rate} + \beta_7 x_{Pressure} x_{Blood} x_{Rate} + \epsilon \\

H_{null} : \beta_5 = \beta_6 = \beta_7 = 0 \\
H_{alternate} : Atleast one of the parameter \beta_5 \neq 0, \beta_6 \neq 0, \beta_7 \neq 0
$$
